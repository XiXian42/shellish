...
const os   = require('os');
const { safeHistoryKey } = require('./history-key');
...
function historyFile(cwd) {
  // per-directory history keyed by safe dirname + stable hash to avoid collisions.
  const safeCwd = safeHistoryKey(cwd);
...
Examples:
  ...
  const platformRules = `- Host OS: ${process.platform === 'win32' ? 'Windows' : process.platform}.
- Use commands appropriate for the execution shell/tools available to you. If a command is unavailable or has incompatible syntax, adapt based on the observed error.`;
  ...
  const deleteSafetyRules = `- File deletion safety: do not permanently delete files.
- When deleting files or directories, prefer the explicit safe-delete command: shellish-trash <path>. It moves targets to the OS Trash/Recycle Bin and may ask the user for confirmation.
- If shellish-trash is unavailable, use bare rm (not /bin/rm, /usr/bin/rm, Remove-Item, del, rmdir, rd, or language-level deletion APIs) so shellish can intercept it where supported.
