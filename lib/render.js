...
const R = ANSI.R, DIM = ANSI.DIM, BOLD = ANSI.BOLD,
      CYAN = ANSI.CYAN, YELLOW = ANSI.YELLOW,
      GREEN = ANSI.GREEN, RED = ANSI.RED;
...
const IS_TTY = process.stdout.isTTY && ANSI.enabled;

// Keep the spinner line within the terminal width: if the line wraps, \r can
// only return to the start of the wrapped row and the rows above become
// permanent garbage. Layout: 2 spaces + frame(1) + 2 spaces + label.
function spinnerLabelBudget() {
...
const spinner = {
  ...
  start(label = '') {
    ...
    this._timer = setInterval(() => {
      ...
      const label = ansiLib.truncateDisplay(this._label, spinnerLabelBudget());
      // \x1b[2K erases the whole line — no stale tail when the label shrinks
      write(`\r\x1b[2K  ${CYAN}${f}${R}  ${DIM}${label}${R}`);
    }, 80);
  },
...
function fmtCmd(cmd) {
  cmd = (cmd || '').replace(/\n/g, ' ').trim();
  return ansiLib.truncateDisplay(cmd, 80);
}
