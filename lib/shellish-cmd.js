...
const CFG_FILE = path.join(os.homedir(), '.config', 'shellish', 'config');
...
function cfgSet(key, value) {
  ...
  let lines = [];
  try { lines = fs.readFileSync(CFG_FILE, 'utf8').split('\n'); } catch { }
...
function cmdUninstallHook() {
  ...
  if (!profilePath || !fs.existsSync(profilePath)) {
    w('  No PowerShell profile found.\n'); return;
  }
  const src = fs.readFileSync(profilePath, 'utf8');
  if (!src.includes('shellish')) {
    w('  No shellish hook found.\n'); return;
  }
