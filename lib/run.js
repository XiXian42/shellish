...
function configCandidates() {
  if (process.env.SHELLISH_CONFIG_FILE) return [process.env.SHELLISH_CONFIG_FILE];
  if (process.env.SHELLISH_CONFIG_DIR) return [path.join(process.env.SHELLISH_CONFIG_DIR, 'config')];
  const legacy = path.join(os.homedir(), '.config', 'shellish', 'config');
  if (process.platform === 'win32') {
    const appdata = process.env.APPDATA || path.join(os.homedir(), 'AppData', 'Roaming');
    return [path.join(appdata, 'shellish', 'config'), legacy];
  }
  return [legacy];
}
...
function getConfirmDanger() {
  for (const file of configCandidates()) {
    try {
      const cfg = fs.readFileSync(file, 'utf8').replace(/^\uFEFF/, '');
