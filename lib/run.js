...
function setupSafeRmBin() {
  ...
  // Session dir holds the rm wrapper + req/res files
...
function promptUser(argsStr) {
  // Delegate to a bash script that reads directly from /dev/tty.
  // execFileSync blocks the event loop — intentional, we need the answer
  // before writing the res file.
  const confirmSh = path.join(LIB_DIR, 'confirm-prompt.sh');
  try {
    const result = require('child_process').execFileSync(
      '/usr/bin/env', ['bash', confirmSh, argsStr],
      { encoding: 'utf8', stdio: ['ignore', 'pipe', 'inherit'] }
    );
    return Promise.resolve(result.trim() || 'N');
  } catch {
  ...
  switch (AGENT) {
    ...
    case 'codex':
      return { cmd: 'codex', args: ['exec', '--json',
        '--dangerously-bypass-approvals-and-sandbox', fullPrompt], env };
...
async function main() {
  ...
  const { cmd, args, env } = agentCmd(fullPrompt, safeRmBin, safeRmInfo);
  ...
  // Pipe agent → renderer
  agent.stdout.pipe(renderer.stdin);
