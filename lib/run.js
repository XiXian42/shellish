...
// Persistent answer reader, shared by every confirmation prompt in this run.
// Why persistent: readline slurps its input's buffered data; with a piped
// stdin (or a fast typist), creating a fresh interface per prompt discards
// any already-buffered next answer when the previous interface closes, and
// the next prompt then sees EOF and reads as deny.
let _promptReader = null;

function getPromptReader() {
  if (_promptReader) return _promptReader;

  // Unix: prefer /dev/tty for isolated reads even when stdin is piped;
  // fall back to stdin (Windows PowerShell stdin is a real tty).
  let input = null;
  let isTty = false;
  if (process.platform !== 'win32') {
    try {
      ...
      input = new net.Socket({ fd: ttyFd, readable: true, writable: false });
      isTty = true;
    } catch { /* no controlling terminal — use stdin */ }
  }

  // `terminal: !!process.stdout.isTTY` (not always true) so IDEs that
  // report isTTY=false don't enter raw mode and swallow keypresses.
  const rl = require('readline').createInterface(isTty
    ? { input }
    : { input: process.stdin, output: process.stdout, terminal: !!process.stdout.isTTY });

  const reader = { rl, queue: [], waiters: [], closed: false };
  rl.on('line', line => {
    const w = reader.waiters.shift();
    if (w) w(line);
    else reader.queue.push(line);   // type-ahead: keep for the next prompt
  });
  rl.on('close', () => {
    reader.closed = true;
    let w;
    while ((w = reader.waiters.shift())) w('N');
  });
  _promptReader = reader;
  return reader;
}

function readAnswerLine() {
  const r = getPromptReader();
  if (r.queue.length) return Promise.resolve(r.queue.shift());
  if (r.closed) return Promise.resolve('N');
  return new Promise(res => r.waiters.push(res));
}

function promptUser(argsStr) {
  const ANSI = require('./ansi').codes(process.stdout);
  const { R, BOLD, DIM, YELLOW, CYAN } = ANSI;
  // The renderer's spinner pauses on .prompting but its current frame is
  // still on this line — erase it so the prompt starts clean.
  const eraseLine = ANSI.enabled ? '\r\x1b[2K' : '';
  ...
  return readAnswerLine().then(ans => {
    process.stdout.write('\n');
    return (ans || 'N').trim();
  });
}
...
async function main() {
  ...
  // Dry-run mode: print what we *would* have spawned, then exit. Useful
  // for CI smoke tests and for users to verify which agent shellish
  // will launch without actually running it.
  if (process.env.SHELLISH_DRY_RUN === '1') {
    process.stdout.write(
      `DRY_RUN agent=${AGENT} cmd=${displayCmd || cmd} ` +
      `args=${JSON.stringify(args)} cwd=${CWD}\n` +
      `DRY_RUN prompt=${fullPrompt}\n`
    );
    return;
  }
  ...
  // JSON lines routinely span chunk boundaries on long outputs, so keep the
  // trailing partial line in a carry buffer instead of dropping both halves.
  let replyBuf = '';
  let lineCarry = '';
  ...
  if (agent.stdout) {
    // StringDecoder: a multi-byte UTF-8 char split across chunks must not
    // be decoded as two replacement chars.
    const { StringDecoder } = require('string_decoder');
    const decoder = new StringDecoder('utf8');
    agent.stdout.on('data', chunk => {
      const lines = (lineCarry + decoder.write(chunk)).split('\n');
      lineCarry = lines.pop();   // last element is a partial line (or '')
      for (const line of lines) teeLine(line);
    });
    agent.stdout.on('end', () => {
      const tail = lineCarry + decoder.end();
      if (tail) teeLine(tail);
      lineCarry = '';
    });
  }
